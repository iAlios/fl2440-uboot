/* S3C24X0 USB Device Controller Driver for u-boot
 *
 * (C) Copyright 2007 by OpenMoko, Inc.
 * Author: Harald Welte <laforge@openmoko.org>
 *
 * based on Linux' S3C24X0_udc.c, which is
...
static inline int fifo_count_out(void)
{
	...
	tmp = inl(S3C24X0_UDC_OUT_FIFO_CNT2_REG) << 8;
	tmp |= inl(S3C24X0_UDC_OUT_FIFO_CNT1_REG);

	return tmp & 0xffff;
}

static const unsigned long ep_fifo_reg[S3C24X0_UDC_NUM_ENDPOINTS] = {
	S3C24X0_UDC_EP0_FIFO_REG,
	S3C24X0_UDC_EP1_FIFO_REG,
	S3C24X0_UDC_EP2_FIFO_REG,
	S3C24X0_UDC_EP3_FIFO_REG,
	S3C24X0_UDC_EP4_FIFO_REG,
};

static int S3C24X0_write_noniso_tx_fifo(struct usb_endpoint_instance *endpoint)
{
	...
	if (!urb || ep >= S3C24X0_UDC_NUM_ENDPOINTS) {
...
static void s3c24x0_deconfigure_device (void)
{
	outl(0, S3C24X0_UDC_EP1_DMA_CON);
	outl(0, S3C24X0_UDC_EP2_DMA_CON);
	outl(0, S3C24X0_UDC_EP3_DMA_CON);
	outl(0, S3C24X0_UDC_EP4_DMA_CON);
}
...
static void udc_set_address(unsigned char address)
{
	...
	outl(address, S3C24X0_UDC_FUNC_ADDR_REG);
}
		   ...
		   unsigned int ep, struct usb_endpoint_instance *endpoint)
{
	...
	if (ep) {
		if ((ep_addr & USB_ENDPOINT_DIR_MASK) == USB_DIR_IN) {
			/* IN endpoint */
			outl(S3C24X0_UDC_ICSR1_FFLUSH|S3C24X0_UDC_ICSR1_CLRDT,
			     S3C24X0_UDC_IN_CSR1_REG);
			outl(S3C24X0_UDC_ICSR2_MODEIN, S3C24X0_UDC_IN_CSR2_REG);
			packet_size = endpoint->tx_packetSize;
			attributes = endpoint->tx_attributes;
		} else {
			/* OUT endpoint */
			outl(S3C24X0_UDC_ICSR1_CLRDT, S3C24X0_UDC_IN_CSR1_REG);
			outl(0, S3C24X0_UDC_IN_CSR2_REG);
			outl(S3C24X0_UDC_OCSR1_FFLUSH|S3C24X0_UDC_OCSR1_CLRDT,
			     S3C24X0_UDC_OUT_CSR1_REG);
			outl(0, S3C24X0_UDC_OUT_CSR2_REG);
	...
	switch (packet_size) {
	case 8:
		maxp = S3C24X0_UDC_MAXP_8;
		break;
	case 16:
		maxp = S3C24X0_UDC_MAXP_16;
		break;
	case 32:
		maxp = S3C24X0_UDC_MAXP_32;
		break;
	default:
	case 64:
		maxp = S3C24X0_UDC_MAXP_64;
		break;
	}
	...
	writel(maxp, S3C24X0_UDC_MAXP_REG);
