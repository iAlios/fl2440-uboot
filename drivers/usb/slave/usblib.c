...
//   EP4: not used
	...
	writeb((EP0_SERVICED_OUT_PKT_RDY | EP0_SERVICED_SETUP_END),
			&usbdevregs->EP0_CSR_IN_CSR1_REG); //EP0:clear OUT_PKT_RDY & SETUP_END
	...
	writeb((EPI_MODE_IN | EPI_IN_DMA_INT_MASK | EPI_BULK),
			&usbdevregs->IN_CSR2_REG);  //IN mode, IN_DMA_INT=masked
	...
	writeb((EPI_FIFO_FLUSH | EPI_CDT | EPI_BULK),
			&usbdevregs->EP0_CSR_IN_CSR1_REG);
	...
	writeb((EP0_INT | EP1_INT | EP2_INT | EP3_INT | EP4_INT),
...
void config_ep3_dma_mode(U32 bufAddr, U32 count) {
	...
	writel(
			((count) | (1 << 31) | (0 << 30) | (1 << 29) | (0 << 28) | (0 << 27)
					| (4 << 24) | (1 << 23) | (1 << 22) | (0 << 20)),
			&dmaregs->dma[2].DCON);
	...
	writeb(
			(readb(&usbdevregs->OUT_CSR2_REG) | EPO_AUTO_CLR
					| EPO_OUT_DMA_INT_MASK), &usbdevregs->OUT_CSR2_REG);
...
void config_ep3_int_mode(void) {
	...
	//DMA channel off
	writeb((readb(&usbdevregs->OUT_CSR2_REG) & ~(EPO_AUTO_CLR)),
			&usbdevregs->OUT_CSR2_REG);
