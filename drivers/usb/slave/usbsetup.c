...
extern volatile int is_usbd_set_configuration;
volatile U8 Rwuen;
...
static const U8 desc_str0[] = { 4, STRING_TYPE, LANGID_EN_L, LANGID_EN_H, };

static const U8 desc_str1[] = {  //Manufacturer
		(0x1c + 2), STRING_TYPE, 'i', 0x0, 'A', 0x0, 'l', 0x0, 'i', 0x0, 'o', 0x0, 's', };

static const U8 desc_str2[] = {  //Product
		(0x26 + 2), STRING_TYPE, 'S', 0x0, '3', 0x0, 'C', 0x0, '2', 0x0, '4', 0x0, '4', 0x0, '0', 0x0,
		' ', 0x0, 'O', 0x0, 'N', 0x0, ' ', 0x0, 'F', 0x0, 'L', 0x0, '2', 0x0, '4', 0x0, '4', 0x0, '0', 0x0,
		' ', 0x0, 'B', 0x0, 'Y', 0x0, ' ', 0x0, 'i', 0x0, '.', 0x0, 'l', 0x0, 'u', 0x0, 'f', 0x0, 'e', 0x0, 'i', 0x0, };
...
//   	DbgPrintf("[STALL]");
	...
	{
		rd_pkt_ep0((U8 *) &descSetup, EP0_PKT_SIZE);
...
// 	    	DbgPrintf("[GDC]");
				...
				if ((descSetup.bLengthL + (descSetup.bLengthH << 8)) > 0x9)
					...
					ep0_state = EP0_STATE_GD_CFG_0; //for WIN98,WIN2K
				else
					ep0_state = EP0_STATE_GD_CFG_ONLY_0; //for WIN2K
				break;
			case STRING_TYPE:
// 	    	DbgPrintf("[GDS]");
				CLR_EP0_OUT_PKT_RDY();
				...
				case 0:
					ep0_state = EP0_STATE_GD_STR_I0;
					break;
				case 1:
					ep0_state = EP0_STATE_GD_STR_I1;
					break;
				case 2:
					ep0_state = EP0_STATE_GD_STR_I2;
					break;
				default:
//	    		DbgPrintf("[UE:STRI?]");
					break;
...
// 	    	DbgPrintf("[GDI]");
				CLR_EP0_OUT_PKT_RDY();
				ep0_state = EP0_STATE_GD_IF_ONLY_0; //for WIN98
				break;
			case ENDPOINT_TYPE:
				//	    	DbgPrintf("[GDE]");
				CLR_EP0_OUT_PKT_RDY();
				...
				case 0:
					ep0_state = EP0_STATE_GD_EP0_ONLY_0;
					break;
				case 1:
					ep0_state = EP0_STATE_GD_EP1_ONLY_0;
					break;
				default:
//	    	    DbgPrintf("[UE:GDE?]");
					break;
...
//            DbgPrintf("[SC]");
		...
		case GET_STATUS:
			...
			case (0x82):
				...
				CLR_EP0_OUT_PKT_RDY();
				if ((descSetup.bIndexL & 0x7f) == 0x00) {
					ep0_state = EP0_GET_STATUS2;
				}

				if ((descSetup.bIndexL & 0x8f) == 0x81) {
					ep0_state = EP0_GET_STATUS3;
				}

				if ((descSetup.bIndexL & 0x8f) == 0x03) {
					ep0_state = EP0_GET_STATUS4;
				}
				break;
...
//            DbgPrintf("[GDD0]");
		wr_pkt_ep0((U8 *) &descDev + 0, 8); //EP0_PKT_SIZE
		SET_EP0_IN_PKT_RDY();
		ep0_state = EP0_STATE_GD_DEV_1;
		break;

	case EP0_STATE_GD_DEV_1:
		//           DbgPrintf("[GDD1]");
		wr_pkt_ep0((U8 *) &descDev + 0x8, 8);
		SET_EP0_IN_PKT_RDY();

		ep0_state = EP0_STATE_GD_DEV_2;
		break;

	case EP0_STATE_GD_DEV_2:
//            DbgPrintf("[GDD2]");
		wr_pkt_ep0((U8 *) &descDev + 0x10, 2);   //8+8+2=0x12
...
//            DbgPrintf("[GDC0]");
		wr_pkt_ep0((U8 *) &descConf + 0, 8); //EP0_PKT_SIZE
		SET_EP0_IN_PKT_RDY();
		ep0_state = EP0_STATE_GD_CFG_1;
		break;

	case EP0_STATE_GD_CFG_1:
//            DbgPrintf("[GDC1]");
		wr_pkt_ep0((U8 *) &descConf + 8, 1);
		wr_pkt_ep0((U8 *) &descIf + 0, 7);
		SET_EP0_IN_PKT_RDY();
		ep0_state = EP0_STATE_GD_CFG_2;
		break;

	case EP0_STATE_GD_CFG_2:
//            DbgPrintf("[GDC2]");
		wr_pkt_ep0((U8 *) &descIf + 7, 2);
		wr_pkt_ep0((U8 *) &descEndpt0 + 0, 6);
		SET_EP0_IN_PKT_RDY();
		ep0_state = EP0_STATE_GD_CFG_3;
		break;

	case EP0_STATE_GD_CFG_3:
		//           DbgPrintf("[GDC3]");
		wr_pkt_ep0((U8 *) &descEndpt0 + 6, 1);
		wr_pkt_ep0((U8 *) &descEndpt1 + 0, 7);
		SET_EP0_IN_PKT_RDY();
		ep0_state = EP0_STATE_GD_CFG_4;
		break;
...
//            DbgPrintf("[GDCO0]");
		wr_pkt_ep0((U8 *) &descConf + 0, 8); //EP0_PKT_SIZE
		SET_EP0_IN_PKT_RDY();
		ep0_state = EP0_STATE_GD_CFG_ONLY_1;
		break;
...
//            DbgPrintf("[GDI0]");
		wr_pkt_ep0((U8 *) &descIf + 0, 8);
		SET_EP0_IN_PKT_RDY();
		ep0_state = EP0_STATE_GD_IF_ONLY_1;
		break;
	case EP0_STATE_GD_IF_ONLY_1:
//           DbgPrintf("[GDI1]");
		wr_pkt_ep0((U8 *) &descIf + 8, 1);
		SET_EP0_INPKTRDY_DATAEND();
		ep0_state = EP0_STATE_INIT;
		break;
...
////////////////////////////////////////////
	...
	case EP0_INTERFACE_GET:
		wr_pkt_ep0((U8 *) &InterfaceGet + 0, 1);
		SET_EP0_INPKTRDY_DATAEND();
		ep0_state = EP0_STATE_INIT;
		break;
...
//            DbgPrintf("[GDS1_%d]",ep0SubState);
		if ((ep0SubState * EP0_PKT_SIZE + EP0_PKT_SIZE) < sizeof(desc_str1)) {
			wr_pkt_ep0((U8 *) desc_str1 + (ep0SubState * EP0_PKT_SIZE),
			EP0_PKT_SIZE);
			SET_EP0_IN_PKT_RDY();
			ep0_state = EP0_STATE_GD_STR_I1;
			ep0SubState++;
			...
			wr_pkt_ep0((U8 *) desc_str1 + (ep0SubState * EP0_PKT_SIZE),
					sizeof(desc_str1) - (ep0SubState * EP0_PKT_SIZE));
			SET_EP0_INPKTRDY_DATAEND();
			ep0_state = EP0_STATE_INIT;
			ep0SubState = 0;
		}
...
//            DbgPrintf("[GDS2_%d]",ep0SubState);
		if ((ep0SubState * EP0_PKT_SIZE + EP0_PKT_SIZE) < sizeof(desc_str2)) {
			wr_pkt_ep0((U8 *) desc_str2 + (ep0SubState * EP0_PKT_SIZE),
			EP0_PKT_SIZE);
			SET_EP0_IN_PKT_RDY();
			ep0_state = EP0_STATE_GD_STR_I2;
			ep0SubState++;
...
//                DbgPrintf("[E]");
			wr_pkt_ep0((U8 *) desc_str2 + (ep0SubState * EP0_PKT_SIZE),
					sizeof(desc_str2) - (ep0SubState * EP0_PKT_SIZE));
			SET_EP0_INPKTRDY_DATAEND();
			ep0_state = EP0_STATE_INIT;
			ep0SubState = 0;
		}
	...
	case EP0_CONFIG_SET:
		wr_pkt_ep0((U8 *) &ConfigSet + 0, 1);
		SET_EP0_INPKTRDY_DATAEND();
		ep0_state = EP0_STATE_INIT;
		break;

	case EP0_GET_STATUS0:
		wr_pkt_ep0((U8 *) &StatusGet + 0, 1);
		SET_EP0_INPKTRDY_DATAEND();
		ep0_state = EP0_STATE_INIT;
		break;

	case EP0_GET_STATUS1:
		wr_pkt_ep0((U8 *) &StatusGet + 1, 1);
		SET_EP0_INPKTRDY_DATAEND();
		ep0_state = EP0_STATE_INIT;
		break;

	case EP0_GET_STATUS2:
		wr_pkt_ep0((U8 *) &StatusGet + 2, 1);
		SET_EP0_INPKTRDY_DATAEND();
		ep0_state = EP0_STATE_INIT;
		break;

	case EP0_GET_STATUS3:
		wr_pkt_ep0((U8 *) &StatusGet + 3, 1);
		SET_EP0_INPKTRDY_DATAEND();
		ep0_state = EP0_STATE_INIT;
		break;

	case EP0_GET_STATUS4:
		wr_pkt_ep0((U8 *) &StatusGet + 4, 1);
		SET_EP0_INPKTRDY_DATAEND();
		ep0_state = EP0_STATE_INIT;
		break;
...
void init_descriptor_table(void) {
	...
	descDev.iSerialNumber = 0xF0EFFE;
